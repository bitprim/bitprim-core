...
using libbitcoin::chain::operation;
...
//"  KP"
uint8_t const protocol_name[] = {0x00, 0x00, 0x4b, 0x50};
constexpr size_t kp_min_size = 4; //TODO(fernando): define
constexpr size_t kp_max_size = to_underlying(opcode::push_four_size); //TODO(fernando): define, not 
...
// ArgumentType<Proc, 0> == size_t
// ArgumentType<Proc, 1> == chain::transaction
// ArgumentType<Proc, 2> == data_chunk
void for_each_keoken_tx(I f, I l, size_t block_height, Proc proc) {
    //precondition: [f, l) is a range of confirmed transactions
    while (f != l) {
        auto const& tx = *f;
        for (auto const& out : tx.outputs()) {
            auto const& keo_data = get_keoken_data(out.script().operations());
            if ( ! keo_data.empty()) {
                proc(block_height, tx, keo_data);
            }
        }
        ++f;
    }
...
// ArgumentType<Proc, 0> == ??
// ArgumentType<Proc, 1> == ??
void for_each_keoken_tx(chain::block const& blk, Proc proc) {
    auto f = blk.transactions().begin();
    auto l = blk.transactions().end();
    ++f;    //skip coinbase transaction

    for_each_keoken_tx(f, l, blk.height(), proc);
}
...
// ************************************************
// Todo esto va en Blockchain o superior
// ************************************************
void usage_from_blockchain() {
...
void hace_algo_con_version(block_height, tx, uint16_t version, data_chunk const& keo_data) {
    switch (version) {
        case 0x00:      //TODO(fernando): enum
            hace_algo_con_version_0(block_height, tx, keo_data);
            break;
        default:
    }
}

void hace_algo_con_version_0(block_height, tx, data_chunk const& keo_data) {
    type = keo_data[2] + keo_data[3];    //TODO(fernando): replace it

    switch (type) {
        case 0x00:      //TODO(fernando): enum, create asset 
            create_asset_version_0(block_height, tx, keo_data);
            break;
        case 0x01:      //TODO(fernando): enum, send tokens
            send_tokens_version_0(block_height, tx, keo_data);
            break;
        default:
    }
}
