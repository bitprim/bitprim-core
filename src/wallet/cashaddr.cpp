 ...
 */
char const* CHARSET = "qpzry9x8gf2tvdw0s3jn54khce6mua7l";

/**
 * The cashaddr character set for decoding.
 */
int8_t const CHARSET_REV[128] = {
 ...
 */
uint64_t poly_mod(data_chunk const& v) {
    /**
    ...
    /**
     * poly_mod computes what value to xor into the final values to make the
...
std::pair<std::string, data_chunk> decode(std::string const& str, std::string const& default_prefix) {
    ...
    // Decode values.
    size_t const valuesSize = str.size() - prefixSize;
    data_chunk values(valuesSize);
